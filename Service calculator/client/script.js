...
function addBundle(bundleName) {
    ...
    // First, clear all selected tasks and bundles
    selectedTasks.clear();
    ...
    // If we're selecting a bundle (not deselecting)
    if (!selectedBundles.has(bundleName)) {
        ...
        // Add active class to the selected bundle button
...
function updateSummary() {
    ...
    // Update comparison table with properly scaled values
    ...
    // Update savings highlight with properly scaled values
...
function updateComparisonTable(diyTime, diyCost, marketCost, jamiezCost, taskCount, recommendedPlan) {
    ...
    // Format time values with proper scaling
    const formatTime = (hours) => {
        // For DIY time, we want to show the actual hours per month/year
        // No need to multiply by multiplier since the hours are already per task
        const totalHours = Math.round(hours);
        return `${totalHours} hours/${timeUnit}`;
    };
    
    // Format cost values with proper scaling
...
function updatePlanRecommendation(plan, taskCount) {
    ...
    // Calculate professional time for the selected tasks
    let totalDIYTime = 0;
    ...
    const professionalTime = totalDIYTime * (2/3);
    
    if (taskCount <= plan.maxTasks) {
        planDetailsEl.textContent = `Perfect fit! This plan covers all ${taskCount} selected tasks (${Math.round(professionalTime)} professional hours).`;
        planDetailsEl.style.color = '#28a745';
    } else {
        const additionalTasks = taskCount - plan.maxTasks;
        planDetailsEl.textContent = `This plan covers ${plan.maxTasks} tasks (${plan.hours} professional hours). ${additionalTasks} additional tasks will require add-on fees.`;
...
function getRecommendedPlan(taskCount, timeHours) {
    // Calculate professional time (2/3 of DIY time)
    const professionalTimeHours = timeHours * (2/3);
    
    // Logic to determine the best plan based on task count and professional time
    if (taskCount <= 4 && professionalTimeHours <= 4) {
        return membershipPlans.essentials;
    } else if (taskCount <= 8 && professionalTimeHours <= 10) {
